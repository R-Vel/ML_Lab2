...
class WhiteBox(BaseEstimator, ClassifierMixin):
    ...
    def fit(
        ...
        if len(self._cat_ind_) > 0:
            ...
            # Categorical features come after numerical features in transformed data
            self._cat_ind_transformed_ = list(range(self._num_features_, 
                                                     self._num_features_ + n_cat_features))
        else:
            self._cat_ind_transformed_ = []
    ...
    def _preprocess(
        ...
        if len(self._cat_ind_) > 0:
            self._preprocessor_ = ColumnTransformer(
                transformers=[
                    ('cat', OneHotEncoder(drop=None, sparse_output=False), self._cat_ind_)
                ],
