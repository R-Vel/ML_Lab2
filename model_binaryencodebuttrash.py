...
class WhiteBox(BaseEstimator, ClassifierMixin):
    ...
    def fit(
        ...
        self._cat_cols_ = X.select_dtypes(include=
                                ['object','category']).columns.tolist()
        
        self._cat_ind_ = [X.columns.get_loc(col) for col in 
                          self._cat_cols_]
    ...
    def fit_predict(
        self, X: Union[np.ndarray, pd.DataFrame], y: Union[np.ndarray, 
        pd.Series]
    ) -> np.ndarray:
        """
    ...
    def _encode_binary_features(self, X):
